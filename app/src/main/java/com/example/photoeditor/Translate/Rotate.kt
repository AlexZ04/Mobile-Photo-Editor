...
class Rotate(_bitmap: Bitmap) {
    ...
    fun rotate(angle: Double) : Bitmap {
        ...
        for (x in 0..<newBitmap.getWidth()) {
            for (y in 0..<newBitmap.getHeight()) {
                ...
                val newColor = if(newX >= 0 && newX < this.bitmap.width
                    && newY >= 0 && newY < this.bitmap.height) this.bitmap.getColor(newX, newY) else Color.BLACK.toColor()
